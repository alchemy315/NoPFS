...
import sys
sys.path.append(str(os.environ['hdmlp_path'])) # 全局变量
import hdmlp
import hdmlp.lib.torch
...
def get_args():
    ...
    # Customize
    parser.add_argument('--world-rank', type=int, required=True,
                        help='The rank of all nodes') # test 加入的全局秩
    parser.add_argument('--world-size', type=int, default=2,
                        help='The num of all nodes') # test node数量
    parser.add_argument('--local-rank', type=int, required=True,
                        help='The rank of GPUs in a node') # test 加入的全局秩
    parser.add_argument('--local-size', type=int, default=2,
                        help='The num of GPUs in a node') # test node数量    
    parser.add_argument('--init-path', type=str,
                        help='The path of ddp init') # 初始化路径
    parser.add_argument('--backend', type=str, default='nccl',
                        help='The backend of ddp init') # ddp后端
    parser.add_argument('--profiler-path', type=str, default='/home/zzp/code/NoPFS/output/log/trace.log',
                        help='the path of profiler output file') # profiler的输出路径
...
def get_local_rank(required=False): # 获取本地进程的rank
    return int(os.environ['get_local_rank'])
...
def get_local_size(required=False):
    return int(os.environ['get_local_size'])
...
def initialize_dist(init_file, rendezvous='file', backend='nccl'): # 分布式初始化方式为tcp或file(默认)
    ...
    world_rank = get_world_rank() # 全局进程的序号
    world_size = get_world_size() # 分布式数量
    init_file = os.path.abspath(init_file) # 转为绝对路径

    if rendezvous == 'tcp':
        dist_url = None
        
        if world_rank == 0: # 头号机器
            import socket
        ...
        print("process_group init",flush=True)
        torch.distributed.init_process_group(
            backend, init_method=dist_url, 
...
def train(args, train_loader, net, scaler, criterion, optimizer,
    ...
    for batch, data in enumerate(train_loader):  # 关键取数据的位置,batch指batch的序号,data为数据数组
        if temp==30:
            prof = profiler.profile(activities=[ torch.profiler.ProfilerActivity.CPU , torch.profiler.ProfilerActivity.CUDA], record_shapes=True) # 定义profiler
            prof.__enter__()
        if temp>=30 and temp<=31: r1 = torch.autograd.profiler.record_function("step"); r1.__enter__()
        ...
        with torch.cuda.amp.autocast(enabled=args.fp16): # 利用with语句，在autocast实例的上下文范围内，进行模型的前向推理和loss计算
            if temp>=30 and temp<=31: r2 = torch.autograd.profiler.record_function("forward"); r2.__enter__()
            output = net(samples)     # net为停下来等待其他机器
            if temp>=30 and temp<=31: r2.__exit__(None,None,None)
            
            if temp>=30 and temp<=31: r3 = torch.autograd.profiler.record_function("loss"); r3.__enter__()
            loss = criterion(output, targets) # loss为一个tensor,它可以记录之前进行过的数学运算，从而进行反向传播
            if temp>=30 and temp<=31: r3.__exit__(None,None,None)

        losses.update(loss, samples.size(0)) # 将本机得到的loss和其他信息进行记录
        
        if temp>=30 and temp<=31: r4 = torch.autograd.profiler.record_function("backward"); r4.__enter__()
        scaler.scale(loss).backward()# 进行反向传播计算梯度
        if temp>=30 and temp<=31: r4.__exit__(None,None,None)

        if temp>=30 and temp<=31: r5 = torch.autograd.profiler.record_function("optimize"); r5.__enter__()
        scaler.step(optimizer) # 梯度有意义时调用optimizer.step()
        if temp>=30 and temp<=31: r5.__exit__(None,None,None)
        
        optimizer.zero_grad() # 清零已经使用过的梯度信息
        
        if temp>=30 and temp<=31: r1.__exit__(None,None,None)
        if temp==31:
            prof.__exit__(None,None,None) # 退出profiler
            prof.export_chrome_trace(os.environ['profiler_path']) # 导出为trace
        temp += 1
...
def main():
    ...
    # 设置全局变量
    os.environ['get_local_size'] = str(args.local_size)
    os.environ['get_local_rank'] = str(args.local_rank)
    os.environ['get_world_size'] = str(args.world_size)
    os.environ['get_world_rank'] = str(args.world_rank)
    os.environ['profiler_path']  = str(args.profiler_path)   # profiler trace的存储路径
    os.environ['init_file_path'] = str(args.init_path)      # 初始化文件的路径
    os.environ['init_backend']   = str(args.backend)          # 后端
    ...
    if args.dist:
        if get_local_size() > get_num_gpus():
            print(get_local_size(),get_num_gpus())
            raise RuntimeError(
                'Do not use more ranks per node than there are GPUs')
        initialize_dist(args.init_path, args.rendezvous, args.backend)
    else: # 非分布式
        if get_world_size() > 1:
    ...
    net = net.to(get_cuda_device()) # 将网络在对应GPU上训练
    ...
    if args.dist:
        net = torch.nn.parallel.DistributedDataParallel( # 多机多卡的分布式训练
            ...
            bucket_cap_mb=args.bucket_cap) # 桶的大小(MB)
    ...
    for epoch in range(args.epochs):
        ...
        train(args, train_loader, net, scaler, criterion, optimizer, log, transforms)
