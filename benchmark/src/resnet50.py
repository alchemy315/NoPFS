...
import sys
sys.path.append("/home/zzp/code/NoPFS")
...
def initialize_dist(init_file, rendezvous='file'): # 分布式初始化方式为tcp或file(默认)
    ...
    if rendezvous == 'tcp':
        ...
        if node_id == 0: # 头号机器
            ...
            with open(init_file, "r") as f: # 读取0号机器的url
                dist_url = f.read()
        print("dist_url=",dist_url,",node_id=",node_id,",num_nodes=",num_nodes) # test
        torch.distributed.init_process_group('gloo', init_method=dist_url,
...
def train(args, train_loader, net, scaler, criterion, optimizer,
    ...
    # args.dali = False # test 该函数内不会运行的部分都注释掉
    # print("0") # test
    net.train()
    # print("0.5") # test
    ...
    for batch, data in enumerate(train_loader):  # 关键取数据的位置,batch指batch的序号,data为数据数组
        ...
        if args.dali and args.no_prefetch:
            ...
            print(targets.size())
        # print("1. do train ",num,",len = ",len(samples)) # test
        ...
        losses.update(loss, samples.size(0)) # 将得到的loss和其他信息进行记录
        ...
        # print("2") # test
        if batch % args.print_freq == 0 and batch != 0: # test 原来 batch % args.print_freq == 0 and batch != 0
            log.log(f'{data_times.mean():.3f}') # test
...
def main():
    ...
    if args.dist:
        ...
        initialize_dist(f'/home/zzp/code/NoPFS/data2/init_{args.job_id}', args.rendezvous) # 使用138机器进行初始化
    else: # 非分布式
        if get_world_size() > 1:
    ...
    for epoch in range(args.epochs):
        # if epoch == 1: # 在运行到一定阶段时
        #     print("I need to sleep about 4 seconds")
        #     time.sleep(4) # 进行睡眠,此时A关掉换成C
        #     # B需要和C进行初始化
        #     initialize_dist(f'./init_{args.job_id}', args.rendezvous) # 再次进行初始化
