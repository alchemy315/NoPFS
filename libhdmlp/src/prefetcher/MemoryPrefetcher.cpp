...
MemoryPrefetcher::MemoryPrefetcher(std::map<std::string, std::string>& backend_options, std::vector<int>::iterator prefetch_start,
    ...
    num_elems = std::distance(prefetch_start, prefetch_end); // 该等级可以存放的元素数量
    file_ends.resize(num_elems, 0);
    {
      int i = 0;
      for (auto iter = prefetch_start; iter != prefetch_end; ++iter, ++i) { // 逐个填入文件的末尾
        int file_id = *iter;
...
void MemoryPrefetcher::prefetch(int thread_id, int storage_class) {
  ...
  while (true) {
    std::unique_lock<std::mutex> lock(prefetcher_mutex); // 上锁
    int idx = prefetch_offset; // 当前需要读取的内容
    if (idx >= num_elems) { // 若已经超过总数
      // Everything is prefetched.
      break; // 就结束循环，结束进程
    }
    ++prefetch_offset;  // Claim this file to prefetch.
    const int file_id = *(prefetch_start + idx);
    if (file_cached[file_id] >= 1) { // 若已经有其他线程进行缓存
      // A different thread cached or is caching this file.
      lock.unlock(); //就解锁
      continue; // 进行下一个
    }
    file_cached[file_id] = 2;  // Mark we're prefetching it. 标记我们要进行cache
    lock.unlock(); // 解锁
    ...
    backend->fetch(file_id, buffer + start); // 取回对应文件
    if (profiling) { // 记录时间
    ...
    lock.unlock();
    metadata_store->insert_cached_file(storage_level, file_id); // 写入元数据
