...
#define PRINT_ALLREDUCE
#ifdef USE_C10D_GLOO

#ifdef PRINT_ALLREDUCE
#include <iostream>   // zzp test
#include <cstdio>   // zzp test
#include <fstream>   // zzp test
#include "unistd.h"  // zzp test
#include <string>  // zzp test
#endif
...
c10::intrusive_ptr<ProcessGroup::Work> ProcessGroupGloo::allreduce(
  ...
  outfile.open("/home/zzp/code/NoPFS/output/log/allreduce",std::ios::out|std::ios::app);
  // outfile << "begin to record allreduce" << std::endl;
  static auto start = std::chrono::system_clock::now(); // 高精度时间
  if (outfile.is_open())   
  { 
    ...
    << "tensor dimension:" << inputs[0].ndimension() << "\n"
    << "tensor size:"; 
    for(int dim=0;dim<inputs[0].ndimension();dim++){
      outfile << inputs[0].size(dim) << " ";
    }
    outfile << "\n}";
    
    // for(int dim=0;dim<tensors[0].ndimension();dim++){
    //   outfile << tensors[0].size(dim) << " ";
    // }
    // outfile << "\n---------------------------------\n";
    outfile.close();  
  }
